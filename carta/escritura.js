...
(() => {
  ...
  class TypeClick {
    ...
    play() {
      ...
      const clickDur = 0.01 + Math.random() * 0.01; // 10–20ms
  ...
  async function playSequence(container, sigEl, options = {}) {
    ...
    for (const line of lines) {
      ...
      // placeInkDropNearLine(card, line);
  ...
  function init() {
    ...
    if (prefersReduce) {
      Array.from(contenido.querySelectorAll(CONFIG.lineSelector)).forEach(
        (l) => {
    ...
    window.escritura = {
      ...
      setVolume(v) {
        clicker.setVolume(Math.max(0, Math.min(1, v)));
      },
      mute() {
